...
class Solution {
    public double findMedianSortedArrays(int[] nums1, int[] nums2) {
        ...
        if((total&1) == 1) {
            return findKthSmallInSortedArrays(nums1,nums2,total/2+1);
        } else {
            double a = findKthSmallInSortedArrays(nums1, nums2,total/2);
            double b = findKthSmallInSortedArrays(nums1, nums2,total/2+1);
            return (a+b)/2;
        }
    }
    double findKthSmallInSortedArrays(int[] nums1, int[] nums2, int k) {
